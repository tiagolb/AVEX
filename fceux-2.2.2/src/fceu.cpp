...
// AVEX: Alocar memoria para a ROM do jogo
...
//AVEX: Carregar o jogo na memoria
void FlushGenieRW(void) {
	int32 x;

	if (RWWrap) { // AVEX: Descartar ultimo bit ; RWWrap no NES; mapeamento bancos de mem
		for (x = 0; x < 0x8000; x++) {
...
//AVEX: Set Write memory function
...
static void AllocBuffers() {
	RAM = (uint8*)FCEU_gmalloc(0x800); //AVEX: 0x800 = RAM size = 2Kb
}
...
void FCEUI_Kill(void) {
	...
	//AVEX: Print Fine Grain Profiling
	// print to file
...
void PowerNES(void) {
	...
	// AVEX: Set ALL the memory blocks at null
	...
	// AVEX: zona de mapeamento/mirroring
...
public:
virtual void Power() {
	// AVEX: da parte da memoria que corresponde a ROM usa-se a CartBR function para ler
